...
{
    public class DriverVehicleCheckRepository : IDriverVehicleCheckRepository {
        private readonly CallidryasDbContext _dbContext;
        private readonly IMapper _mapper;

        public DriverVehicleCheckRepository(CallidryasDbContext dbContext, IMapper mapper) {
        ...
        public async Task<Response> GetAsync() {
            ...
            try {
                var list = await _dbContext.DriverVehicleChecks.OrderByDescending(d=>d.Id).ToListAsync();
                oResponse.Data = list;
                oResponse.Success = 1;
            }
            catch (Exception ex){
        ...
        //             .Take(pageSize)
        //             .Select(d => new DriverVehicleCheckDTO {
        ...
        public async Task<Response> GetOneAsync(int Id) {
            ...
            try {
                ...
                var driverVehicleChecks = await _dbContext.DriverVehicleChecks.FirstOrDefaultAsync(d => d.Id == Id);

                if (driverVehicleChecks == null) {
                    oResponse.Message = "driverVehicleChecks not found.";
                    oResponse.Success = 0;
                } else {
                    oResponse.Data = driverVehicleChecks;
                    oResponse.Success = 1;
                }
            } catch (DbUpdateException dbEx) {
                oResponse.Success = 0;
                oResponse.Message = $"Error en la base de datos al agregar el driverVehicleChecks: {dbEx.Message}";
            } catch (Exception ex) {
                oResponse.Success = 0;
                oResponse.Message = $"Error inesperado al agregar el driverVehicleChecks: {ex.Message}";
            }
        ...
        public async Task<Response> AddAsync(DriverVehicleCheckDTO model) {
            ...
            try {
                ...
                var driverEntity = _mapper.Map<DriverVehicleCheck>(model);

                // Agregar la entidad y guardar cambios dentro de una transacción controlada
                await _dbContext.DriverVehicleChecks.AddAsync(driverEntity);
        ...
        public async Task<Response> UpdateAsync(DriverVehicleCheckDTO model, int Id) {
            var oResponse = new Response();
            ...
            try {
                ...
                var existingVehicleDriverCheck = await _dbContext.DriverVehicleChecks.FindAsync(Id);
                if (existingVehicleDriverCheck == null) {
                    oResponse.Success = 0;
                ...
                _mapper.Map(model, existingVehicleDriverCheck);

                // Guardar los cambios solo si hubo modificaciones
                if (_dbContext.Entry(existingVehicleDriverCheck).State == EntityState.Modified) {
                    await _dbContext.SaveChangesAsync();
                    oResponse.Success = 1;
        ...
        public async Task<Response> DeleteAsync(int Id) {
            ...
            try {
                ...
                var existingVehicleDriverCheck = await _dbContext.Drivers.FindAsync(Id);
                ...
                _dbContext.Drivers.Remove(existingVehicleDriverCheck);
                await _dbContext.SaveChangesAsync();
